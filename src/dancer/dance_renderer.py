...
REQUIRED_BONES = (
    "Hips", "Chest", "Neck", "Head", "UpperArm_L", "UpperArm_R",
    "LowerArm_L", "LowerArm_R", "UpperLeg_L", "UpperLeg_R", "LowerLeg_L", "LowerLeg_R",
)
...
def _import_siroino(path: Path) -> tuple[bpy.types.Object, list[bpy.types.Object]]:
    ...
    if len(candidates) != 1:
        raise RuntimeError(f"expected one Siroino armature with required bones, found {len(candidates)}")
...
def _import_bvh(path: Path, target_armature: bpy.types.Object) -> bpy.types.Object:
    ...
    candidates = [
        obj for obj in bpy.context.scene.objects
        if obj.type == "ARMATURE" and obj != target_armature and all(name in obj.data.bones for name in source_bones)
    ]
    if len(candidates) != 1:
        raise RuntimeError(f"expected one BVH armature with mapped bones, found {len(candidates)}")
    return candidates[0]
...
def _parse_bvh_timing(path: Path) -> tuple[int, float]:
    ...
    time_match = re.search(r"^Frame Time:\s*([0-9.eE+-]+)\s*$", motion[1], re.MULTILINE)
...
def _configure_scene(
    meshes: list[bpy.types.Object], *, width: int, height: int, camera_preset: str,
    ...
    camera.location = center + direction * distance + Vector((0.0, 0.0, 0.10 * body_height))
    camera.data.lens = 52
    _look_at(camera, center + Vector((0.0, 0.0, 0.04 * body_height)))
    scene.camera = camera
    base_camera_location = camera.location.copy()

    bpy.ops.mesh.primitive_plane_add(size=max(body_height * 5.0, 1.0), location=(center.x, center.y, minimum.z - 0.015 * body_height))
...
def _framing_margin(meshes: list[bpy.types.Object], camera: bpy.types.Object) -> float:
    scene = bpy.context.scene
    ...
    for source in meshes:
        ...
        try:
            ...
            for vertex in mesh.vertices[::step]:
                coordinate = world_to_camera_view(scene, camera, evaluated.matrix_world @ vertex.co)
                if coordinate.z <= 0:
                    margin = min(margin, -1.0)
                else:
                    margin = min(margin, coordinate.x, 1.0 - coordinate.x, coordinate.y, 1.0 - coordinate.y)
                points_seen += 1
        finally:
...
def _retarget_setup(source: bpy.types.Object, target: bpy.types.Object) -> tuple[dict[str, object], dict[str, Matrix]]:
    scene = bpy.context.scene
    scene.frame_set(1)
...
def _apply_motion_frame(
    source: bpy.types.Object, target: bpy.types.Object, source_frame: float,
    source_baseline: dict[str, object], basis_alignment: dict[str, Matrix],
) -> None:
...
def render_dance(
    output_dir: str | Path, *, motion_id: str, duration_seconds: float, fps: int,
    size: int | None = None, width: int | None = None, height: int | None = None,
    ...
    camera, camera_target, base_camera_location, body_height = _configure_scene(meshes, width=width, height=height, camera_preset=camera_preset)
    ...
    for frame_index in range(frame_count):
        ...
        _apply_motion_frame(source_armature, target_armature, source_frame, source_baseline, basis_alignment)
        if camera_preset == "dolly":
            ...
            camera.location = base_camera_location + toward_target * (0.10 * body_height * progress)
    ...
    subprocess.run(
        ["ffmpeg", "-y", "-loglevel", "error", "-framerate", str(fps), "-i", str(frame_dir / "%04d.png"), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-movflags", "+faststart", str(output_path)],
        check=True,
    )
    shutil.rmtree(work)
