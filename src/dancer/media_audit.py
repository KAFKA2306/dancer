...
def audit_media(
    ...
    *,
    ...
    frame_hashes = {
        ...
        for line in frame_md5.stdout.splitlines()
        if line and not line.startswith("#")
    }
    unique_frame_hashes = len(frame_hashes)
    if unique_frame_hashes < 2:
        reasons.append(
            "video has no proven frame-to-frame motion: "
            f"unique frame hashes={unique_frame_hashes}"
        )
    ...
    black_duration = sum(
        ...
        for value in re.findall(
            r"black_duration:([0-9.]+)", black.stderr or ""
        )
    )
    if duration > 0 and black_duration / duration > 0.08:
        reasons.append(
            "black frames exceed threshold: "
            f"{black_duration:.3f}s of {duration:.3f}s"
        )
    ...
    if mean_volume is None or max_volume is None:
        ...
        if mean_volume < -45.0:
            reasons.append(
                f"audio is effectively silent: mean {mean_volume:.1f} dB"
            )
        if max_volume > -0.1:
            reasons.append(
                f"audio peak is too close to clipping: {max_volume:.1f} dB"
            )
    ...
    return MediaAudit(
        ...
        unique_video_frame_hashes=unique_frame_hashes,
