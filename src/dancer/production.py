...
from .audio import AudioArtifact, beat_aligned_duration, get_audio_track, materialize_audio, mux_audio, track_provenance
from .dance_renderer import CAMERA_PRESETS, IMAGE2OUTFIT_COMMIT, SIROINO_BLOB_SHA, SIROINO_LICENSE, SIROINO_TERMS_URL, SIROINO_URL
...
def _candidate_score(artifact: GeneratedArtifact, audit: MediaAudit) -> float:
    if not audit.passed or artifact.framing_margin < 0:
        return float("-inf")
    return artifact.framing_margin - min(artifact.camera_max_step_per_frame, 1.0) * 0.001
    ...
    if artifact.framing_margin < 0:
        audit = MediaAudit(
            **{
                **asdict(audit),
                "passed": False,
                "reasons": tuple(audit.reasons) + (f"body framing crosses image boundary: margin={artifact.framing_margin:.5f}",),
            }
        )
    ...
    *,
    ...
    runner(
        [
            ...
            "-ss", f"{timestamp_seconds:.3f}", "-i", str(video_path),
            "-frames:v", "1",
    ...
    if destination.stat().st_size > YOUTUBE_THUMBNAIL_MAX_BYTES:
        raise ValueError(f"thumbnail exceeds YouTube 2 MB limit: {destination.stat().st_size} bytes")
    return {
...
def _metadata(*, motion: DanceMotion, audio: AudioArtifact, motion_source: dict[str, Any]) -> dict[str, object]:
    title = f"{motion.category} — SiroinoSotai dance ({motion.id})"[:100]
    attribution = ""
    if audio.track.attribution_required:
        attribution = f"\nAudio attribution: {audio.track.creator} — {audio.track.title} ({audio.track.license_url})"
    description = (
        ...
        f"Audio license: {audio.track.license_name} — {audio.track.license_url}{attribution}"
    )
    return {
        "title": title,
        "description": description[:5000],
        "tags": ["3D dance", "Blender", "SiroinoSotai", motion.category, motion.id],
        "thumbnail_title": title,
        "sources": {"motion": motion.url, "avatar": SIROINO_URL, "audio": audio.track.source_page_url},
    }


def _compliance(*, motion: DanceMotion, audio: AudioArtifact, motion_source: dict[str, Any], metadata: dict[str, object]) -> dict[str, object]:
    checks = {
        "motion_source_https": motion.url.startswith("https://"),
        "motion_terms_present": str(motion_source.get("cmu_terms_url", "")).startswith("https://") and str(motion_source.get("conversion_terms_url", "")).startswith("https://"),
        "avatar_source_pinned": IMAGE2OUTFIT_COMMIT in SIROINO_URL and bool(SIROINO_BLOB_SHA),
        "avatar_terms_present": SIROINO_TERMS_URL.startswith("https://") and SIROINO_LICENSE == "CC0-1.0",
        ...
        "required_attribution_present": (not audio.track.attribution_required or audio.track.creator in str(metadata.get("description") or "")),
    }
    return {
        "passed": all(checks.values()),
        "checks": checks,
        "contains_synthetic_media": False,
        "synthetic_media_reason": "The output is a visibly stylized 3D character animation; this pipeline does not create a realistic depiction of a real person, place, scene, or event.",
...
def _handoff_to_yt3(
    *, manifest_path: Path, yt3_root: Path, profile: str,
    runner: Callable[..., subprocess.CompletedProcess[str]] = subprocess.run,
) -> dict[str, object]:
    ...
    imported = runner(
        ["bun", "src/scripts/import_dancer_artifact.ts", str(manifest_path), profile],
        cwd=yt3_root, capture_output=True, text=True, check=True,
    )
    lines = [line.strip() for line in imported.stdout.splitlines() if line.strip().startswith("{")]
    if not lines:
    ...
    if not run_id or not publish_video_path:
        raise RuntimeError("YT3 import result is missing run_id or publish_video_path")
    runner(["task", "publish", f"PROFILE={profile}", "--", run_id, publish_video_path], cwd=yt3_root, check=True)
...
def run_production(
    ...
    *,
    ...
    if candidates < 1 or candidates > len(CAMERA_PRESETS):
        raise ValueError(f"candidates must be between 1 and {len(CAMERA_PRESETS)}")
    if fps <= 0 or width <= 0 or height <= 0 or width % 2 or height % 2:
        raise ValueError("fps must be positive and dimensions must be positive even integers")
    ...
    if existing is None:
        ...
        for index, camera_preset in enumerate(CAMERA_PRESETS[:candidates]):
            candidate_results.append(
                _render_candidate(
                    candidate_dir=run_dir / "candidates" / f"{index:02d}-{camera_preset}",
                    motion=motion, audio=audio, duration_seconds=duration, fps=fps,
                    width=width, height=height, camera_preset=camera_preset,
                )
            )
        valid = [candidate for candidate in candidate_results if bool(candidate.get("audit", {}).get("passed")) and float(candidate.get("score", float("-inf"))) != float("-inf")]
        if not valid:
        ...
        thumbnail = _make_thumbnail(selected_path, run_dir / "presentation" / "thumbnail.jpg", timestamp_seconds=max(0.0, duration * 0.35))
        metadata = _metadata(motion=motion, audio=audio, motion_source=motion_source)
        metadata_path = run_dir / "presentation" / "metadata.json"
        _json_dump(metadata_path, metadata)
        compliance = _compliance(motion=motion, audio=audio, motion_source=motion_source, metadata=metadata)
        ...
        manifest = {
            ...
            "inputs": key_input,
            "motion": {"id": motion.id, "category": motion.category, "source_url": motion.url, "source": motion_source},
            "avatar": {"source_url": SIROINO_URL, "source_commit": IMAGE2OUTFIT_COMMIT, "source_blob_sha": SIROINO_BLOB_SHA, "terms_url": SIROINO_TERMS_URL, "license": SIROINO_LICENSE},
    ...
    if yt3_root is not None and publish_profile is not None:
        publication = _handoff_to_yt3(manifest_path=manifest_path, yt3_root=Path(yt3_root).resolve(), profile=publish_profile)
